...
│   │   Note: Playwright MCP must be available for these tools to work.
...
7. **Always use `filename` parameter** on Playwright tools (`browser_snapshot`, `browser_console_messages`, `browser_network_requests`) when Playwright MCP is available. Without it, the full output enters context.
...
**When Playwright MCP is available and a task involves page inspection, ALWAYS route through file → sandbox.**
