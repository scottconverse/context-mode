/**
 * Shared session directive builder for context-mode hooks.
 *
 * Contains: groupEvents, writeSessionEventsFile, buildSessionDirective,
...
// Uses db.getEvents() since our SessionDB uses a private #db field.
...
// Falls back to scanning all sessions via getLatestSessionId helper.
export function getLatestSessionEvents(db) {
  // Try to get events from the most recently started session that has events.
  // Our SessionDB doesn't expose raw .db, so we use the public getEvents API
  // and rely on the caller to have ensured the session exists.
  // For resume, we ask the DB for the latest session ID via a helper if available,
  // otherwise use db.getLatestSessionId() if exposed, or return [] as safe fallback.
  if (typeof db.getLatestSessionId === "function") {
    const sessionId = db.getLatestSessionId();
    if (!sessionId) return [];
    return db.getEvents(sessionId);
  }
  // Safe fallback — session-start will proceed without directive
  return [];
}
